...
<div class="btn-group margin-bottom pull-right">
    ...
    <button type="button" class="btn btn-success btn-xs margin-r-5 filter"
            value="{{ \App\Models\Vote::STATE_NORMAL }}">正常
    </button>
    <button type="button" class="btn btn-danger btn-xs margin-r-5 filter"
            value="{{ \App\Models\Vote::STATE_DELETED }}">已删除
    </button>
    <button type="button" class="btn btn-primary btn-xs margin-r-5 recommend"
            value="{{ \App\Models\Survey::TOP_TRUE }}" title="推荐到轮播图">推荐
    </button>
...
<script>
    /* 新增 */
    function create() {
        window.location.href = '/votes/create'
    }
