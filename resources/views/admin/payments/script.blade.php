<script>
    ...
    function titleFormatter(value, row, index) {
        return '<a href="/payments/detail-' + row.id + '.html" target="_blank">' + row.title + '</a>' +
    ...
    function updateRow(field, row, old, $el) {
        $.ajax({
            url: '/admin/payments/' + row.id + '/save',
            type: 'post',
    ...
    window.actionEvents = {
        ...
        'click .comment': function (e, value, row, index) {
            ...
            var url = '/admin/payments/comments/' + row.id;
            $.ajax({
                url: url,
                type: "get",
                data: {'_token': '{{ csrf_token() }}'},
