...
@section('content')
                ...
                <div class="col-md-9">
                    <div class="box box-info">
                        <div class="box-header">
                            <button class="btn btn-primary btn-xs margin-r-5 margin-t-5" onclick="window.location.href='/admin/features/column/create/{{ $category_id }}';"> 新增子专题</button>
                        </div>
                        <div class="box-body">
                            ...
                            <table id="table"
                                   data-toggle="table"
                                   data-url="/admin/features/table/{{ $category_id }}/{{ $time }}">
                                <thead>
                                <tr>
    ...
    <script>
        //查询所有父类ID为0，最新一个月份的专题
        $.getJSON('/admin/features/categories', function (data) {
            $('#tree').treeview({
                showTags: true,
                ...
                onNodeSelected: function (event, data) {
                    location.href = '/admin/features/column?category_id=' + data.id + '&time='+data.time;
                }
            });

            if (getTimeIndex(getQueryString('time'), data) >= 0) {
        ...
        window.actionEvents = {
            'click .edit': function (e, value, row, index) {
                window.location.href = '/admin/features/column/' + row.id + '/edit';
            },
