<script>
    ...
    function titleFormatter(value, row, index) {
        return [
            '<a href="/admin/features/' + row.id + '" target="_blank">' + row.title + '</a>',
        ]
    }
    ...
    function updateRow(field, row, old, $el) {
        $.ajax({
            url: '/admin/features/' + row.id + '/save',
    ...
    window.actionEvents = {
        ...
        'click .comment': function (e, value, row, index) {
            ...
            var url = '/admin/features/comments/' + row.id;
            $.ajax({
                url: url,
                type: "get",
