...
class SpecialController extends BaseController
{
    ...
    public function detail()
    {
        ...
        $key = "Special-detail-$id";

        return cache_remember($key, 1, function () use ($id) {
            $special = Special::findOrFail($id);
            $site = $special->site;
            $theme = $special->site->mobile_theme;
            $special->content = replace_content_url($special->content);
            return view("themes.$theme.specials.detail", compact('site', 'special'))->__toString();
        });
    }
    ...
    public function share()
    {
        ...
        return cache_remember($key, 1, function () use ($id) {
            ...
            $special->content = replace_content_url($special->content);
            $share = 1;
            return view("themes.$theme.specials.detail", compact('site', 'special', 'share'))->__toString();
        });
    }
}